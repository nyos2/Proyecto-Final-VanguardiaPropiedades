...
public class PropertyService {
    ...
    @Transactional
    public void editarProperty(String id, int precio, String tipo, String idUser) throws MyException {
        validar(precio,tipo);

        Optional<UserEntity> respuesta = userRepository.findById(idUser);
        Optional<Property> respuestaP= propertyRepository.findById(id);
        ...
        if (respuestaP.isPresent()){
            Property property= respuestaP.get();
            property.setPrecio(precio);
            property.setTipo(Tipo.valueOf(tipo));
            property.setUsuario(usuario);
            property.setEstado(true);
